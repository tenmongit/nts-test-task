...
ip -br a
```
![srv1: статические адреса на месте](screenshots/srv1-ip-addr.png)
![srv2: статические адреса на месте](screenshots/srv2-ip-addr.png)

```bash
ip -d link show vlan10 #в выводе vlan protocol 802.1Q id 10
```
![srv1: vlan10, protocol 802.1Q id 10](screenshots/srv1-vlan-details.png)
![srv2: vlan10, protocol 802.1Q id 10](screenshots/srv2-vlan-details.png)

```bash
tcpdump -ni enp1s0 -e vlan # кадр 102 байта против 98 без метки
```
![Метки в пакете](screenshots/vlan-tcpdump-tagged.png)
![Без меток в пакете](screenshots/vlan-tcpdump-untagged.png)

Пинг из VLAN 10 в VLAN 20 не проходит
![Изоляция](screenshots/vlan-isolation.png)

```bash
reboot
ip -br a
```
![Переживает перезагрузку](screenshots/reboot-static-persists.png)
